...
__export(index_exports, {
  OAuthInvalidTokenError: () => OAuthInvalidTokenError,
  PayloadMcpOAuthError: () => PayloadMcpOAuthError,
  payloadMcpOAuth: () => payloadMcpOAuth
});
...
function hashToken(plaintext) {
  return import_crypto.default.createHmac("sha256", getPepper()).update(plaintext).digest("hex");
}
...
function verifyCsrfToken(token, userId, clientId, redirectUri, codeChallenge, maxAgeMs = DEFAULT_MAX_AGE_MS) {
  ...
  try {
    ...
    return import_crypto2.default.timingSafeEqual(tokenBuf, expectedBuf);
  } catch {
    return false;
  }
}
...
function verifyPkce(verifier, challenge, method) {
  ...
  const computed = import_crypto3.default.createHash("sha256").update(verifier).digest("base64url");
  ...
  if (computedBuf.length !== challengeBuf.length) {
    import_crypto3.default.timingSafeEqual(computedBuf, computedBuf);
    return false;
  }
  return import_crypto3.default.timingSafeEqual(computedBuf, challengeBuf);
}
...
function generateToken(kind) {
  ...
  const entropy = import_crypto4.default.randomBytes(32).toString("base64url");
...
function makeRegisterHandler() {
  return async (req) => {
    ...
    const clientId = (0, import_crypto5.randomUUID)();
...
var import_internal = require("@payloadcms/plugin-mcp/internal");
...
function installOverrideGetAuthorizedMCP(mcpPluginOptions, userCollection) {
  mcpPluginOptions.overrideGetAuthorizedMCP = async ({
    ...
    let items = await (0, import_internal.filterMCPItems)({
...
async function defaultAuthorize({
  ...
  if (req.headers) {
    ...
    if (hasAuthorization && !req.user) {
      throw new import_payload.UnauthorizedError(req.t);
    }
  }
  return {
    items: await (0, import_internal.filterMCPItems)({
      items: pluginConfig.items,
      overrideAccess,
      req
    }),
...
function wrapMcpEndpointHandler(original, issuer, canonicalPathname) {
  ...
  return async (req) => {
    ...
    try {
      ...
      if (err instanceof import_payload.UnauthorizedError) {
        return new Response(null, {
          status: 401,
          headers: {
...
function warnIfVersionUntested() {
  try {
    const pkg = require("@payloadcms/plugin-mcp/package.json");
